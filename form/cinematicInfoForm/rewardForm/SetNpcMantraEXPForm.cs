...
namespace 侠之道mod制作器
{
    public partial class SetNpcMantraEXPForm : Form
    {
        public bool isAdd;
        public object obj;
        public SetNpcMantraEXPForm()
        {
            InitializeComponent();

            initMethodComboBox();
        }
        public SetNpcMantraEXPForm(object obj, bool isAdd) : this()
        {
            this.obj = obj;
            this.isAdd = isAdd;
            ...
            if (!string.IsNullOrEmpty(fields))
            {
                ...
                mantraIdTextBox.Text = fieldsList[2].Trim();
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            ...
            string tag = "\"SetNpcMantraEXP\" : " + ((ComboBoxItem)methodComboBox.SelectedItem).key + ", " + valueNumericUpDown.Text + ", " + "\"" + mantraIdTextBox.Text + "\"" + ", " + "\"" + npcIdTextBox.Text + "\"";
            string text = Text + ":" + DataManager.getCharacterInfoRemark(npcIdTextBox.Text) + " 的 " + DataManager.getMantraName(mantraIdTextBox.Text) + " 的经验" + " " + methodComboBox.Text + (((ComboBoxItem)methodComboBox.SelectedItem).key == ((int)Method.Clear).ToString() ? "" : (" " + valueNumericUpDown.Text));
