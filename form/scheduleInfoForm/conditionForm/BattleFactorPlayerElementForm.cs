using Heluo.Data;
using System;
using System.Windows.Forms;

namespace 侠之道mod制作器
{
    public partial class BattleFactorPlayerElementForm : Form
    {
        public bool isAdd;
        ListViewItem lvi = null;
        public BattleFactorPlayerElementForm()
        {
            InitializeComponent();
            initElementComboBox();
        }

        public BattleFactorPlayerElementForm(ListViewItem lvi, bool isAdd, Form owner) : this()
        {
            Owner = owner;
            this.lvi = lvi;

            string fields = lvi.Tag.ToString().Split(':')[1];
            if (!string.IsNullOrEmpty(fields))
            {
                string[] fieldsList = Utils.getFieldsList(fields);

                for (int i = 0; i < elementComboBox.Items.Count; i++)
                {
                    if (((ComboBoxItem)elementComboBox.Items[i]).key == fieldsList[0].Trim())
                    {
                        elementComboBox.SelectedIndex = i;
                        break;
                    }
                }
                if (fieldsList[1] == "True")
                {
                    isReverseCheckBox.Checked = true;
                }

            }

            nextNumericUpDown.Value = int.Parse(lvi.SubItems[2].Text);


            this.isAdd = isAdd;
        }

        public void initElementComboBox()
        {
            elementComboBox.DisplayMember = "value";
            elementComboBox.ValueMember = "key";
            foreach (Element temp in Enum.GetValues(typeof(Element)))
            {
                ComboBoxItem cbi = new ComboBoxItem(((int)temp).ToString(), EnumData.GetDisplayName(temp));
                elementComboBox.Items.Add(cbi);
            }
        }

        private void okButton_Click(object sender, EventArgs e)
        {
            if (elementComboBox.SelectedIndex == -1)
            {
                MessageBox.Show("请选择功体");
                return;
            }

            ScheduleInfoForm scheduleInfoForm = (ScheduleInfoForm)Owner;
            ListView scheduleListView = scheduleInfoForm.getScheduleListView();
            lvi.Tag = "\\\"BattleFactorPlayerElement\\\" : " + ((ComboBoxItem)elementComboBox.SelectedItem).key + ", " + (isReverseCheckBox.Checked ? "True" : "False");
            lvi.SubItems[1].Text = Text + ":" + (isReverseCheckBox.Checked ? "不是" : "是") + " " + elementComboBox.Text;
            lvi.SubItems[2].Text = nextNumericUpDown.Text;

            if (isAdd)
            {
                scheduleListView.Items.Add(lvi);
            }
            Close();

            scheduleInfoForm.createScheduleTree();
        }

        private void cancelButton_Click(object sender, EventArgs e)
        {
            Close();
        }

        private void selectNextButton_Click(object sender, EventArgs e)
        {
            SelectScheduleNodeForm form = new SelectScheduleNodeForm(this, nextNumericUpDown);
            form.ShowDialog();
        }
    }
}
