...
namespace 侠之道mod制作器
{
    public partial class BufferPositiveStatePromoteActionForm : Form
    {
        public bool isAdd;
        public BufferPositiveStatePromoteActionForm()
        {
            InitializeComponent();
        ...
        public BufferPositiveStatePromoteActionForm(string tag, bool isAdd, Form owner) : this()
        {
            Owner = owner;
            ...
            if (!string.IsNullOrEmpty(fields))
            {
                ...
                {
                    if (((ComboBoxItem)statusComboBox.Items[i]).key == fieldsList[4].Trim())
                    {
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            ...
            currentNode.Tag = "\"BufferPositiveStatePromoteAction\" : " + ((ComboBoxItem)propertyComboBox.SelectedItem).key + ", "
                + ((ComboBoxItem)methodComboBox.SelectedItem).key + ", " + float.Parse(valueNumericUpDown.Text).ToString("0.00000") + ", " + float.Parse(valueLimitNumericUpDown.Text).ToString("0.00000") + ", " + ((ComboBoxItem)statusComboBox.SelectedItem).key;
            ...
            currentNode.Text = "正面状态重数提升属性:" + "每有1重 " + statusComboBox.Text + " " + bufferStr;
            Close();
        }
