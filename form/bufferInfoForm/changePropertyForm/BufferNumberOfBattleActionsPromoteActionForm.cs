...
namespace 侠之道mod制作器
{
    public partial class BufferNumberOfBattleActionsPromoteActionForm : Form
    {
        public bool isAdd;
        public BufferNumberOfBattleActionsPromoteActionForm()
        {
            InitializeComponent();
        ...
        public BufferNumberOfBattleActionsPromoteActionForm(string tag, bool isAdd, Form owner) : this()
        {
            Owner = owner;
            ...
            if (!string.IsNullOrEmpty(fields))
            {
                ...
                for (int i = 0; i < actionsComboBox.Items.Count; i++)
                {
                    if (((ComboBoxItem)actionsComboBox.Items[i]).key == fieldsList[4].Trim())
                    {
                        actionsComboBox.SelectedIndex = i;
                        break;
                    }
                }
            }
        ...
        public void initActionsComboBox()
        {
            actionsComboBox.DisplayMember = "value";
            actionsComboBox.ValueMember = "key";
            foreach (BattleActions temp in Enum.GetValues(typeof(BattleActions)))
            {
                ComboBoxItem cbi = new ComboBoxItem(((int)temp).ToString(), EnumData.GetDisplayName(temp));
                actionsComboBox.Items.Add(cbi);
            }
        }
        ...
        private void okButton_Click(object sender, EventArgs e)
        {
            ...
            if (actionsComboBox.SelectedIndex == -1)
            {
                MessageBox.Show("请选择行动类型");
                return;
            }
            ...
            currentNode.Tag = "\"BufferNumberOfBattleActionsPromoteAction\" : " + ((ComboBoxItem)propertyComboBox.SelectedItem).key + ", "
                + ((ComboBoxItem)methodComboBox.SelectedItem).key + ", " + float.Parse(valueNumericUpDown.Text).ToString("0.00000") + ", " + float.Parse(valueLimitNumericUpDown.Text).ToString("0.00000") + ", " + ((ComboBoxItem)actionsComboBox.SelectedItem).key;
            ...
            currentNode.Text = "战斗行动数值数提升属性:" + "每1 " + actionsComboBox.Text + " " + bufferStr;
            Close();
        }
